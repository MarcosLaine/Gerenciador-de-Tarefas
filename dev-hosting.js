...
// Mostra as URLs primeiro
displayURLs();

// Aguarda um pouco para o usuário ver as URLs
setTimeout(() => {
  console.log('🚀 Iniciando servidores...\n');
  
  // Executa o concurrently
  const concurrently = spawn('npx', ['concurrently', '-n', 'BACKEND,FRONTEND', '-c', 'blue,green', 'npm run dev:backend:hosting', 'npm run dev:frontend:hosting'], {
    stdio: 'inherit',
    shell: true,
    cwd: process.cwd()
  });
